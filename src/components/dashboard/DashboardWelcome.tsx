...
export function DashboardWelcome() {
  ...
  const visibleShortcuts = shortcuts.map(shortcut => {
    if (shortcut.href === '/teams' && !canAccess('squad')) return { ...shortcut, title: 'Leagues & divisions', description: 'Discover your cricket community', href: '/browse-leagues' };
    if (shortcut.href === '/rankings' && !canAccess('powerindex')) return { ...shortcut, title: 'Your inbox', description: 'Stay connected with your community', href: '/inbox' };
    return shortcut;
  });
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_280px]">
      <section className="hero-inverse relative isolate min-h-[310px] overflow-hidden rounded-[26px] border border-white/10 bg-[#101810] text-white">
        <Image src="/images/cricket-hero.png" alt="Cricket batsman playing a shot on a sunlit field" fill priority sizes="(max-width: 768px) 100vw, 70vw" className="object-cover object-right"/><div className="hero-scrim absolute inset-0"/>
        <div className="relative flex min-h-[310px] max-w-[78%] sm:max-w-[68%] flex-col items-start justify-center p-6 sm:p-8">
          ...
          <Link href="/matches" className="mt-6 inline-flex items-center gap-3 rounded-xl bg-[#D7F900] px-5 py-3 text-sm font-semibold text-[#101610] hover:bg-[#e4ff4d]">Explore matches<ArrowUpRight size={17}/></Link>
        </div>
      </section>
      <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
        {visibleShortcuts.map(({title, description, href, icon: Icon}) => <Link key={href} href={href} className="group flex items-center gap-4 rounded-[22px] border border-border bg-card p-5 transition-colors hover:border-primary/40">
