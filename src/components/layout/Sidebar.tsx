...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    const onEscape = (event: KeyboardEvent) => { if (event.key === 'Escape') setOpenMobile(false); };
  ...
  return <>
    ...
    <nav aria-label="Main navigation" inert={isMobile && !openMobile ? true : undefined} className={cn('fixed inset-y-0 left-0 z-50 flex flex-col border-r border-border bg-card transition-[width,transform] duration-200', collapsed ? 'w-20' : 'w-[260px]', openMobile ? 'translate-x-0' : '-translate-x-full md:translate-x-0')}>
      <div className="flex h-24 shrink-0 items-center justify-between gap-2 px-5">
        <Link href="/home" onClick={close} aria-label="SCRBRD dashboard" className="flex items-center gap-2.5">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-primary text-primary-foreground"><Zap size={23} strokeWidth={2.5} /></span>
          {!collapsed && <span className="text-xl font-bold tracking-tight">SCRBRD<span className="block text-[10px] font-medium tracking-[.22em] text-muted-foreground">YOUR GAME. CONNECTED.</span></span>}
        </Link>
        {!collapsed && <button onClick={isMobile ? close : toggleSidebar} aria-label={isMobile ? 'Close navigation' : 'Collapse navigation'} className="rounded-lg p-1.5 text-muted-foreground hover:bg-secondary">{isMobile ? <X size={18}/> : <PanelLeftClose size={16}/>}</button>}
      </div>
      {collapsed && <button onClick={toggleSidebar} aria-label="Expand navigation" className="mx-auto mb-4 rounded-xl p-2 text-muted-foreground hover:bg-secondary"><PanelLeftOpen size={18}/></button>}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-4">
        ...
        {navGroups.map(group => <CollapsibleNavGroup key={group.key} group={group} isCollapsed={collapsed} filterQuery={filterQuery} onLinkClick={close}/>)}
        {filterQuery && !navGroups.some(group => group.label.toLowerCase().includes(filterQuery.toLowerCase()) || group.links.some(link => link.label.toLowerCase().includes(filterQuery.toLowerCase()))) && <p role="status" className="p-4 text-sm text-muted-foreground">No workspaces found. Try a different name.</p>}
      </div>
