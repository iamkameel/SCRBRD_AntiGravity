...
export function LiveHeroCard({
  ...
  return (
    <Card className="relative overflow-hidden bg-card border-border/50 shadow-sm py-6 animate-in fade-in zoom-in-95 duration-500">
      ...
      <CardContent className="flex flex-col items-center justify-center p-0 relative z-10 px-6 text-center">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mb-4">
          <Badge variant="outline" className="font-black text-[11px] uppercase tracking-[0.2em] border-white/10 text-muted-foreground bg-white/5">
            {liveScore?.inningsNumber === 2 ? '2nd' : '1st'} Innings · {ci?.battingTeamId === match.homeTeamId ? match.homeTeamName : match.awayTeamName}
          </Badge>
          
          {sig?.phase && (
            <Badge variant="outline" className={`font-black text-[11px] uppercase tracking-widest ${phaseColClass}`}>
          ...
          {freeHit && (
             <Badge variant="outline" className="bg-violet-500/20 text-violet-800 dark:text-violet-300 border-violet-500/30 font-black text-[11px] uppercase tracking-widest animate-pulse">
        ...
        <div className="font-black text-6xl md:text-7xl tracking-tighter leading-none text-foreground drop-shadow-lg mb-4">
          {ci?.runs ?? 0}<span className="text-muted-foreground text-4xl md:text-5xl tracking-tight">/{ci?.wickets ?? 0}</span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-4 text-xs font-bold uppercase tracking-widest text-muted-foreground">
          ...
          {isChase && ci?.target && (
            <>
              <span className="hidden sm:inline">•</span>
              <span className="text-amber-800 dark:text-amber-300">RRR <span className="font-mono ml-1">{rrrStr(ci.target, ci.runs ?? 0, ci.balls ?? 0, overs * 6)}</span></span>
            </>
          )}
        </div>
        ...
        {isChase && ci?.target && (
          <div className="mt-6 flex flex-col items-center animate-in slide-in-from-bottom-2">
            <div className="inline-flex items-center gap-3 px-6 py-3 rounded-2xl bg-amber-500/10 border border-amber-500/20">
              ...
              <span className="text-xl font-mono font-black text-amber-800 dark:text-amber-300 drop-shadow-[0_0_10px_rgba(245,158,11,0.5)]">{ci.target}</span>
