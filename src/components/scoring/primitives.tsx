...
export function Lbl({ children, color }: { children: React.ReactNode; color?: string }) {
  return (
    <div style={{
      fontFamily: D.head, fontSize: '10px', fontWeight: 700, letterSpacing: '0.15em',
      textTransform: 'uppercase', color: color || D.textMuted
    }}>{children}</div>
  );
}
...
export function Badge({ children, color }: { children: React.ReactNode; color: string }) {
  return (
    <span style={{
      fontFamily: D.head, fontSize: '9px', fontWeight: 700, letterSpacing: '0.12em',
      textTransform: 'uppercase', padding: '3px 8px', borderRadius: D.pill,
      background: `${color}1e`, color, border: `1px solid ${color}30`, flexShrink: 0
    }}>{children}</span>
  );
}
...
export function Btn({ children, onClick, disabled, variant = 'primary', full, sx }: {
  ...
  return (
    <button className="sh-press" disabled={!!disabled} onClick={!disabled ? onClick : undefined}
      style={{
        ...v, padding: '12px 20px', borderRadius: D.pill, cursor: disabled ? 'not-allowed' : 'pointer',
...
export function Sheet({ children, title, accent, onClose }: { children: React.ReactNode; title?: string; accent?: string; onClose: () => void }) {
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
      ...
      <div className="sh-slide-up" style={{
        position: 'relative', background: D.glass, backdropFilter: 'blur(28px) saturate(1.8)',
...
export function LiveDot() {
  return (
    <span className="sh-live-dot" style={{
