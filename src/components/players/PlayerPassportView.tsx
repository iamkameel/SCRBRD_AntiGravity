...
const StarRating = ({ rating = 4 }) => (
  <div className="flex gap-1">
    {[1, 2, 3, 4, 5].map((s) => (
      <Star 
        key={s} 
        className={cn("h-3 w-3", s <= rating ? "text-primary fill-primary" : "text-white/10")} 
      />
    ))}
  </div>
);
...
const TimelineStep = ({ age, years, school, team, active = false }: { age: string, years: string, school: string, team: string, active?: boolean }) => (
  <div className={cn(
    ...
    <div className={cn(
      ...
      active ? "bg-primary text-black border-primary" : "bg-[#0A0A0A] text-white/40 border-white/10"
    )}>
      {age}
    </div>
    <div className="mt-2 space-y-1">
      <div className={cn("text-[10px] font-black tracking-widest", active ? "text-primary" : "text-white/30")} style={{ fontFamily: D.mono }}>{years}</div>
      <div className="text-xs font-bold text-white/90 truncate">{school}</div>
      <div className="text-[9px] font-bold text-white/40 uppercase tracking-widest truncate">{team}</div>
    </div>
    {/* Connection chevron (pseudo logic) */}
    <div className="absolute top-1/2 -right-3 -translate-y-1/2 text-white/10 hidden lg:block">
...
const StatBlock = ({ label, value, sub, color ="primary" }: { label: string, value: string | number, sub?: string, color?: "primary" | "blue" }) => (
  <div className="text-center p-4">
    <div className="text-[8px] font-black uppercase tracking-[0.2em] text-white/30 mb-2" style={{ fontFamily: D.mono }}>{label}</div>
    <div className={cn("text-3xl font-black tracking-tighter", color === "primary" ? "text-white" : "text-blue-400")} style={{ fontFamily: D.head }}>{value}</div>
    {sub && <div className="text-[9px] font-bold text-white/40 uppercase tracking-widest mt-1">{sub}</div>}
  </div>
);
...
export function PlayerPassportView({ player, profile }: PlayerPassportViewProps) {
  ...
  return (
    <div className="space-y-10 sh-fade-in pb-20">
      ...
      <div className="relative rounded-[3rem] border border-white/10 bg-[#0A0A0A] overflow-hidden group">
        ...
        <div className="relative p-12 flex flex-col lg:flex-row gap-16">
          {/* Portrait & Security */}
          <div className="flex flex-col items-center gap-8">
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="text-center">
                <div className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30" style={{ fontFamily: D.mono }}>Biometric Status</div>
                <div className="text-[11px] font-black text-emerald-500 uppercase tracking-widest">Secured & Verified</div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-12">
            <div className="flex items-start justify-between">
              ...
              <div className="flex flex-col items-end gap-2">
                <div className="h-20 w-20 rounded-full border-4 border-primary/20 flex items-center justify-center bg-white/5 shadow-inner">
            ...
            {/* Horizontal Timeline */}
            <div className="space-y-4">
              <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20" style={{ fontFamily: D.mono }}>Professional Trajectory</h3>
            ...
            <div className="grid grid-cols-2 gap-8 pt-8 border-t border-white/5">
               <div className="rounded-3xl bg-white/[0.03] border border-white/5 overflow-hidden">
                  <div className="bg-white/5 px-6 py-2 border-b border-white/5">
               ...
               <div className="rounded-3xl bg-white/[0.03] border border-white/5 overflow-hidden">
                  <div className="bg-white/5 px-6 py-2 border-b border-white/5">
                     <span className="text-[10px] font-black uppercase tracking-widest text-[#60a5fa]" style={{ fontFamily: D.mono }}>Bowling Core</span>
                  </div>
                  <div className="flex items-center justify-around p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 h-full">
        ...
        <div className="rounded-[3rem] border border-white/10 bg-[#0A0A0A] p-10 flex flex-col justify-between group overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-primary/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
          <div className="relative space-y-8">
            <h3 className="text-xl font-black text-white" style={{ fontFamily: D.head }}>SKILLS & <span className="text-primary">DEVELOPMENT</span></h3>
            <PassportRadarChart />
            <div className="space-y-6">
               <div className="space-y-4">
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-white/30" style={{ fontFamily: D.mono }}>Development Focus</h4>
                  <ul className="space-y-3">
                     {[
                       ...
                       <li key={i} className="flex items-center gap-3 text-[11px] font-bold text-white/70">
        ...
        {/* Form & Trends Center */}
        <div className="rounded-[3rem] border border-white/10 bg-[#0A0A0A] p-10 flex flex-col group overflow-hidden">
          <div className="relative space-y-8 flex-1">
            <div className="flex justify-between items-start">
              <h3 className="text-xl font-black text-white" style={{ fontFamily: D.head }}>FORM & <span className="text-primary">TRENDS</span></h3>
              <div className="text-right">
                <div className="text-[10px] font-black uppercase tracking-widest text-primary mb-1">Form Index</div>
                <div className="text-4xl font-black text-white" style={{ fontFamily: D.head }}>7.6 <span className="text-primary text-sm">↑ 12%</span></div>
              </div>
            </div>
        ...
        <div className="rounded-[3rem] border border-white/10 bg-[#0A0A0A] p-10 flex flex-col gap-10 group overflow-hidden">
           <ScoutingIntel />
           
           <div className="space-y-6">
              <h3 className="text-xl font-black text-white" style={{ fontFamily: D.head }}>HONOURS & <span className="text-primary">BADGES</span></h3>
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={i} className="rounded-3xl border border-white/10 bg-white/[0.02] flex flex-col items-center justify-center p-6 text-center group/stamp hover:bg-white/[0.05] transition-all">
                    <stamp.icon className={cn("h-10 w-10 mb-2 opacity-30 group-hover/stamp:opacity-100 transition-opacity", stamp.color)} />
                    <div className="text-[9px] font-black uppercase tracking-widest text-white/90 leading-tight">{stamp.label}</div>
                  </div>
                ))}
              </div>
           </div>
           ...
           <div className="space-y-4">
              <div className="text-[10px] font-black uppercase tracking-widest text-white/20 text-center" style={{ fontFamily: D.mono }}>Registry Status</div>
              <div className="flex items-center gap-4 p-5 rounded-3xl bg-primary/5 border border-primary/20">
                 ...
                 <div>
                    ...
                    <div className="text-[9px] font-medium text-white/40">Cleared for Regional Representation</div>
                 </div>
              </div>
           </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between pt-10 border-t border-white/5 opacity-40 hover:opacity-100 transition-opacity">
         <div className="flex items-center gap-2">
            <Fingerprint className="h-5 w-5 text-primary" />
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-white/60" style={{ fontFamily: D.mono }}>Official Record: {player.id?.toUpperCase()}</span>
         </div>
          <div className="text-right">
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-white/60" style={{ fontFamily: D.mono }}>POWERED BY SCRBRD OS INTEL CORE</span>
          </div>
       </div>
    </div>
  );
