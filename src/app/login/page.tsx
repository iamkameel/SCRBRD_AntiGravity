...
function LoginForm() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background relative overflow-hidden p-6 selection:bg-primary/30">
      {/* Cricket Stadium Background with Overlay */}
      <div className="absolute inset-0 z-0">
        <Image
          src="/images/cricket-bg.png"
          alt="Cricket Stadium Background"
          fill
          priority
          className="object-cover object-center filter brightness-[0.35] contrast-[1.15] scale-105"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-background via-background/80 to-background/40" />
        <div className="absolute inset-0 bg-grid-white/[0.03] bg-[size:50px_50px]" />
      </div>
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-primary/10 blur-[150px] rounded-full opacity-40 pointer-events-none z-0" />
      <div className="absolute top-0 right-0 p-8 flex items-center gap-2 text-muted-foreground z-10">
      ...
      <div className="w-full max-w-[440px] relative z-10 animate-slide-in-up">
        <div className="flex flex-col items-center mb-10">
          ...
          <p className="text-muted-foreground text-sm font-medium">Sign in to your cricket workspace.</p>
        </div>
        ...
        <Card className="glass-card border-white/5 shadow-2xl overflow-hidden">
          <CardContent className="pt-8 px-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <Button
                type="submit"
                className="w-full h-12 text-sm font-semibold bg-primary hover:bg-primary/90 shadow-xl shadow-primary/10 transition-all duration-300"
                disabled={loading}
              >
                {loading ? (
        ...
        {/* Security Footer */}
        <div className="mt-12 flex items-center justify-center gap-4 transition-all duration-500">
          <div className="flex items-center gap-1">
            <ShieldCheck className="h-3 w-3" />
            <span className="text-xs text-muted-foreground">Secure sign in</span>
          </div>
          <div className="w-1 h-1 rounded-full bg-white/20" />
          <span className="text-xs text-muted-foreground">Your team. Your season.</span>
        </div>
      </div>
    </div>
  );
